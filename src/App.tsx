...
export default function App() {
  const [view, setView] = useState<'landing' | 'checking' | 'success' | 'error'>('landing');
  ...
  const handleClaim = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const usernameRef = doc(db, 'waitlist', normalizedUsername);
      let docSnap;
      try {
        docSnap = await getDoc(usernameRef);
      } catch (error) {
        handleFirestoreError(error, OperationType.GET, `waitlist/${normalizedUsername}`);
        return;
      }

      if (docSnap && docSnap.exists()) {
        // Username is taken
        setErrorMessage('Someone beat you to it. That username is already claimed in our database. Try another variation to secure your spot.');
        setView('error');
      } else {
        ...
        try {
          ...
          handleFirestoreError(error, OperationType.CREATE, `waitlist/${normalizedUsername}`);
        }
      }
    } catch (error: any) {
      console.error("Error claiming username:", error);
      setErrorMessage('An error occurred while connecting to the database. Please try again later.');
      setView('error');
    }
  };
          ...
          <AnimatePresence mode="wait">
            {view === 'landing' && (
              ...
              >
                ...
                <div className="flex flex-col items-center gap-[24px] text-center">
                  ...
                  >
                    Bypass the noise and gatekeepers. Our curated network connects ambitious founders, investors, and creators directly with the individuals who can accelerate their vision.
                  </motion.p>
                </div>
                ...
                <motion.form 
                  ...
                  className="relative rounded-full p-[1px] bg-gradient-to-b from-white/20 to-white/5 overflow-visible group w-full max-w-[440px] focus-within:from-white/40 focus-within:to-white/10 hover:from-white/30 hover:to-white/10 transition-all duration-500"
                ...
                >
                  ...
                  <div className="relative z-10 bg-black/80 backdrop-blur-2xl rounded-full p-1.5 md:p-2 flex items-center shadow-[0_0_30px_-10px_rgba(255,255,255,0.05)] group-hover:shadow-[0_0_40px_-10px_rgba(255,255,255,0.1)] group-focus-within:shadow-[0_0_60px_-15px_rgba(255,255,255,0.2)] transition-all duration-500">
                    ...
                    <button type="submit" className="relative bg-white text-black rounded-full h-[36px] w-[36px] md:h-[42px] md:w-[42px] group-hover:w-[90px] md:group-hover:w-[100px] group-focus-within:w-[90px] md:group-focus-within:w-[100px] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] cursor-pointer shrink-0 overflow-hidden">
                      ...
                      >
                        <path className="transform origin-left transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] scale-x-0 opacity-0 group-hover:scale-x-100 group-hover:opacity-100 group-focus-within:scale-x-100 group-focus-within:opacity-100" d="M5 12h14"/>
                        <path className="transform transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] -translate-x-2 group-hover:translate-x-0 group-focus-within:translate-x-0" d="m12 5 7 7-7 7"/>
                      </svg>
                    </button>
                  </div>
                </motion.form>
            ...
            {view === 'checking' && (
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  <h2 className="text-2xl md:text-3xl font-medium text-white">Verifying availability</h2>
                  <p className="text-white/50 text-[15px]">Checking @{username} against our database...</p>
                </div>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <p className="text-white/60 text-[15px] leading-relaxed">
                    username has been claimed by you. You are on the waitlist.
                  </p>
                </div>
                <button 
                  onClick={resetFlow}
            ...
            {view === 'error' && (
              ...
              >
                ...
                <div className="flex flex-col gap-4">
                  <h2 className="text-3xl md:text-4xl font-medium text-white">@{username} is taken.</h2>
                  <p className="text-white/60 text-[15px] leading-relaxed">
